...
class WebRTCService extends ChangeNotifier {
  ...
  void _connectSupabaseRealtime() {
    ...
    _channel = supabase.channel(channelName, opts: const RealtimeChannelConfig(ack: true));

    _channel!
        .onBroadcast(
          event: 'signal',
          callback: (payload) {
            if (payload['senderId'] != _selfId) {
              //print('Received signal: ${payload}');
  ...
  Future<void> _sendSignal(Map<String, dynamic> signal) async {
    ...
    try {
      await _channel!.sendBroadcastMessage(event: 'signal', payload: {'senderId': _selfId, 'data': signal});
    } catch (e) {
